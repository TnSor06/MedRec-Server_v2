...
async function createPatientRecord(parent, args, { prisma, request }, info) {
  const result = await createPatientRecordSchema.validate({
    case: args.data.case,
  ...
  if (prevPatientRecords.length === 0) {
    ...
    let recordNum = prevPatientRecords[0].recordId.substr(
      patientCase.caseId.length
    );
...
async function viewPatientRecord(parent, args, { prisma, request }, info) {
...
export { createPatientRecord, viewPatientRecord };
