...
async function mePatient(parent, args, { prisma, request }, info) {
  const userData = getUserData(request);
...
async function viewPatient(parent, args, { prisma, request }, info) {
  const userData = getUserData(request);
...
async function registerPatient(parent, args, { prisma, request }, info) {
  const result = await registerPatientSchema.validate({
...
async function updatePatient(parent, args, { prisma, request }, info) {
  const userData = getUserData(request);
  ...
  const result = await updatePatientSchema.validate({
    password: args.data.password,
    address: args.data.address,
    religion: args.data.religion,
    maritalStatus: args.data.maritalStatus,
...
export { mePatient, registerPatient, updatePatient, viewPatient, Patient };
