...
async function meDatabaseAdmin(parent, args, { prisma, request }, info) {
...
async function approvePatient(parent, args, { prisma, request }, info) {
  const userData = getUserData(request);
...
async function approveMedicalPractitioner(
  parent,
  args,
  { prisma, request },
  info
  ...
  if (
    databaseAdminHospital[0].hospital.id === medicalPractitioner.hospital.id
  ) {
...
async function registerDatabaseAdmin(parent, args, { prisma, request }, info) {
  const result = await registerDatabaseAdminSchema.validate({
...
async function updateDatabaseAdmin(parent, args, { prisma, request }, info) {
  const userData = getUserData(request);
  ...
  const result = await updateDatabaseAdminSchema.validate({
    address: args.data.address,
    contact: args.data.contact,
    password: args.data.password,
  });
